...
df1 = pd.read_csv('wikipediaNodes.csv', sep=',')
df2 = pd.read_csv('wikipediaEdges.csv', sep=',')
...
for idx, row in df2.iterrows():
    G2.add_edge(row['SourceLanguageCode'], row['TargetLanguageCode'], weight=row['Coocurrences'])
...
centrality = nx.eigenvector_centrality(G2, max_iter=200, tol=1e-11, weight='weight')
...
#	print('%s %0.8f'%(key, values['centrality']))
...
plt.xlabel("log10(Wikipedia Eigenvector Centrality)", fontsize=24)
plt.show()
